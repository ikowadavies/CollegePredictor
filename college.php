...
<body>
	...
	<div class="container">
		<?php
		if(isset($_GET['cid'])) {
			$con = mysqli_connect('localhost', 'root', '', 'college-predictor');

			$cid = mysqli_real_escape_string($con, $_GET['cid']);
			$query = "SELECT * FROM `clg_marks` WHERE cid=".$cid;
			$result = mysqli_query($con, $query);


			if($result) {
					$row=mysqli_fetch_row($result);
					echo '<h1>'.$row[1].'<small> ('.$row[2].')</small></h1><hr><p><strong>Cut-off: </strong>'.$row[3].'<br>'.$row[4].'</p><p><strong><u>Contact</u></strong><br>Website: <a href="http://localhost/CollegePredictor/Colleges/website"'.$row[5].'">http://www.'.$row[5].'</a><br>Phone: (+260)'.$row[6].'</p>';
